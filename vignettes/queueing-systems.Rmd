...
# It matches (approx.) the theoretical value
mm1.T <- mm1.N / lambda
...
mm13.trajectory <- create_trajectory() %>%
...
mm13.env <- simmer() %>%
  ...
  add_generator("arrival", mm13.trajectory, function() rexp(1, lambda)) %>%
  run(until=2000)
```
...
```{r}
mm13.arrivals <- get_mon_arrivals(mm13.env)
mm13.arrivals %>%
...
```{r}
mm13.t_system <- mm13.arrivals$end_time - mm13.arrivals$start_time
# Comparison with M/M/1 times
qqplot(mm1.t_system, mm13.t_system)
abline(0, 1, lty=2, col="red")
```
