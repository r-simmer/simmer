...
These examples are adapted from the Python package 'SimPy', [here](https://simpy.readthedocs.io/en/latest/examples). Users familiar with SimPy may find these examples helpful for transitioning to `simmer`. Some very basic material is not covered. Beginners should first read the vignettes [_Introduction to simmer_](A-introduction.html), [_Terminology_](B-terminology.html), [_Advanced Trajectory Usage_](C-trajectories.html) and [_The Bank Tutorial_](D-bank-1.html).
...
moviegoer <- trajectory() %>%
  ...
  select(function() movies[get_attribute(env, "movie")]) %>%
  # set reneging condition
  renege_if(function() paste0(movies[get_attribute(env, "movie")], " sold out")) %>%
  # leave immediately if the movie was already sold out
  leave(function() get_capacity(env, movies[get_attribute(env, "movie")]) == 0) %>%
  # wait for my turn
  seize("counter", 1) %>%
  # buy tickets
  ...
  branch(function() get_server_count(env, movies[get_attribute(env, "movie")]) > (TICKETS - 2),
         continue = TRUE,
         # trigger the "sold out" event for the movie
         trajectory() %>%
           set_capacity_selected(0) %>%
           send(function() paste0(movies[get_attribute(env, "movie")], " sold out"))
  ) %>%
  timeout(1) %>%
  release("counter", 1) %>%
  # watch the movie
