...
The `set_attribute(., key, value)` method sets the `value` of an arrival's attribute `key`. Be aware that `value` can only be numeric.
...
The `send(., n, timeout, permanent, name, rule)` m
...
This is not very useful if nobody is listening. Arrivals can subscribe to signals and (optionally) assign a handler with the `trap(., signals, handler)`. In the following example, an arrival subscribes to a signal and blocks until its reception using the `wait(.)` method.
...
Note that, while inside a batch, all the signals subscribed before entering the batch are ignored. Thus, the following batch will block indifinitely:
