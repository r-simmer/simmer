...
TANK <- GAS_STATION_SIZE
signal <- "tank refilled"

refuelling <- create_trajectory() %>%
  branch(function(attr) FUEL_TANK_SIZE - attr["level"] > TANK, continue = TRUE,
         create_trajectory() %>%
           trap(signal) %>%
           wait() %>%
           untrap(signal)
...
controller <- create_trajectory() %>%
  branch(function() TANK / GAS_STATION_SIZE * 100 < THRESHOLD, continue = TRUE,
         create_trajectory() %>%
