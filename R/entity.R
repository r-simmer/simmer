...
Resource <- R6Class("Resource", inherit = Entity,
  public = list(
    ...
    seize = function(arrival, amount) {
      if (private$mon) self$observe(self$sim$now)
      
      if (private$room_in_server(amount)) {
        private$server_count <- private$server_count + amount
    ...
    release = function(arrival, amount) {
      ...
      if (length(private$queue_count)) {
        ...
        if (!is.null(another_arrival)) {
          ...
          private$server_count <- private$server_count + another_amount
          self$sim$schedule(0, another_arrival)
        }
      }
      return(0)
    },
  ...
  private = list(
    ...
    room_in_server = function(amount) {
      return(private$server_count + amount <= private$capacity)
    },
...
Generator <- R6Class("Generator", inherit = Process,
  public = list(
    initialize = function(sim, name_prefix, first_event, dist) {
  ...
  public = list(
    ...
    activate = function() {
      ...
      if (is.null(current_event))
        ...
        self$activity_time <- self$activity_time + 
