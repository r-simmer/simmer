...
##        R CMD build simmer
## Usage: docker run --rm -ti -v $(pwd):/mnt r-patched /bin/bash -c "_R_CHECK_FORCE_SUGGESTS_=false RP CMD check --as-cran /mnt/simmer_x.x.x.tar.gz"
...
RUN RPscript -e 'install.packages(c("Rcpp", "BH", "R6", "magrittr", "codetools", "testthat", "knitr"))'
